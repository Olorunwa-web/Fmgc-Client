...
const Hygiene = ({searchTerm}) => {
    ...
    const filtered = hygiene.filter((product) =>
    product.title.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const itemsPerPages = 12;
  const start = (currentPage - 1) * itemsPerPages;
  const paginatedhygiene = filtered.slice(start, start + itemsPerPages);
